...
surroundingElement:b});({data:a}=yield a.json());a.forEach(d=>{console.log(d);createRatingElements(b,d)})}}else throw new CannotFetchDataError({message:"Could not extract data-user-account or data-surrounding-element-id from script. Please provide them!",surroundingElement:null});}})}
...
d.textContent=(new Date(c.createdAt)).toLocaleDateString("de-DE");b.appendChild(d);b=document.createElement("div");b.classList.add("rating-comment");b.textContent=c.comment;e.appendChild(b);a.appendChild(e)}
