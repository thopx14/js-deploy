var __awaiter=this&&this.__awaiter||function(a,c,d,b){function e(f){return f instanceof d?f:new d(function(g){g(f)})}return new (d||=Promise)(function(f,g){function k(h){try{l(b.next(h))}catch(m){g(m)}}function n(h){try{l(b["throw"](h))}catch(m){g(m)}}function l(h){h.done?f(h.value):e(h.value).then(k,n)}l((b=b.apply(a,c||[])).next())})};const thisScriptElement=document.getElementById("cituro-ratings-script");
...
function fetchSummaryDataAndDisplayItOnScreen(a,c){return __awaiter(this,void 0,void 0,function*(){var d=yield fetch(`https://app.cituro.com/api/ratings/${a}/summary`);if(!d.ok)throw new CannotFetchDataError({message:d.statusText,surroundingElement:c});var {data:b}=yield d.json();d=b.count;if(0<d){b=b.average;const f=document.createElement("div");f.classList.add("rating-summary-container");const g=document.createElement("p");g.classList.add("rating-summary-text");g.textContent=`${b.toFixed(2)} aus 5 Sternen `;
const k=document.createElement("span");k.classList.add("rating-stars");var e=generateRatingStarsSvg(b);k.append(...e);e=document.createElement("a");e.setAttribute("href","https://app.cituro.com/ratings/claudia-knoblich-ayurveda-yoga-ulm");e.setAttribute("target","_blank");e.textContent=`(${d} Bewertungen auf cituro.com)`;g.appendChild(e);f.appendChild(g);f.appendChild(k);c.appendChild(f);generateJsonLdScriptForRatingSummary(b.toFixed(2),d)}})}
